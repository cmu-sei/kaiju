...
import kaiju.analyzers.DisasmImprovementsAnalyzer;
import kaiju.common.InsnControlFlowGraph;
import kaiju.common.InsnControlFlowGraphDebugVisitor;
import kaiju.fnhash.FnHashOptions;
 ...
 */
public class FnHashAnalyzer extends AbstractAnalyzer {
    ...
    public void optionsChanged(Options options, Program program) {
        ...
        if (options.contains(FnHashOptions.LOG_LEVEL_OPTION_NAME)) {
            logger.setLogLevel(options.getEnum(FnHashOptions.LOG_LEVEL_OPTION_NAME, MultiLogLevel.WARN));
        }
    }
    ...
    public boolean added(Program program, AddressSetView set, TaskMonitor monitor, MessageLog log)
        ...
        int fncount = 0;
        if (fiter == null) {
            logger.warn(this, "No functions found?");
        } else {
            while (fiter.hasNext()) {
                ...
                try {
                    ...
                    logger.error(this, "Error while computing function hashes", e);
                }
            }
        }
        logger.info(this, "Fn2Hash analysis complete: Found hashes for " + fncount + " functions.");
        //logger.debug(this, ManualViewerCommandWrappedOption().getCommandString);
    ...
    private FnHashSaveable runOneFn(Function function, Program program, TaskMonitor monitor) throws Exception {
        ...
        try {
            ...
            logger.warn(this, "Exception caught while computing hashes", e);
