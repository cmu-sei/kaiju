...
import kaiju.tools.disasm.impl.X86ImproverStrategy;
 ...
 */
public class DisasmImprover {

    private DisasmStrategy improver;
    private Program program;
    private TaskMonitor monitor;
    
    public DisasmImprover(Program currentProgram, TaskMonitor currentMonitor) {
        program = currentProgram;
        monitor = currentMonitor;
    }

    // sets the strategy so appropriate implementation class is called
    public void setImproverStrategy(DisasmStrategy strategy) {
        this.improver = strategy;
    }
    
    // convenience function allowing to specify string representation
    // of an architecture rather than a specific class
    public void setImproverStrategy(String strategy) throws InvalidImproverStrategyException {
    
        String arch = strategy.toLowerCase();
        
        if (arch.equals("x86")) {
            setImproverStrategy(new X86ImproverStrategy(program, monitor));
        } else {
            throw new InvalidImproverStrategyException();
        }
    
    }
    
    // CodeUnit insn?
    // convenience function to access the DisasmStrategy function
    public Pair<AddressRange, Integer> analyzeGap(AddressRange range) {
        return improver.analyzeGap(range);
    }
