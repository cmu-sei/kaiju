...
public class AshrExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BitVecExpr instantiate(GhiHornContext ctx) {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Ashr operation expects bitvector term:" + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Ashr operation expects bitvector term:" + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVASHR(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public AshrExpression(HornExpression lhs, HornExpression rhs) {
        this.lhs = lhs;
        this.rhs = rhs;
    }
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof AshrExpression))
            return false;
        AshrExpression other = (AshrExpression) obj;
        if (lhs == null) {
            if (other.lhs != null)
                return false;
