...
public class BoolOrExpression implements HornExpression {
    ...
    public BoolExpr instantiate(GhiHornContext ctx) throws Z3Exception {
        ...
        Verify.verify(countOfNonBoolTypes == 0, "Or requires boolean types for terms");

        final List<BoolExpr> termExprs = terms.stream()
                .map(t -> (BoolExpr) t.instantiate(ctx))
                .collect(Collectors.toList());

        return ctx.mkOr(termExprs.toArray(new BoolExpr[0]));
    }
     ...
     * @param rhs
     */
    public BoolOrExpression(List<? extends HornExpression> terms) {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof BoolOrExpression))
            return false;
        BoolOrExpression other = (BoolOrExpression) obj;
        if (terms == null) {
            if (other.terms != null)
                return false;
