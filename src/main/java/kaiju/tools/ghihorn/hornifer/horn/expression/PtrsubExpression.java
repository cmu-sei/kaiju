...
public class PtrsubExpression implements HornExpression {
    ...
    public Expr<? extends Sort> instantiate(GhiHornContext ctx) {
        ...
        Verify.verify(offset.getType() == GhiHornType.BitVec, "Ptrsub requires bitvector offset term: " + offset);
        Verify.verify(pointer.getType() == GhiHornType.BitVec, "Ptrsub requires bitvector pointer term: " + pointer);
        ...
        // indicates
        // explicitly that input0 is a reference to a structured data-type and one of
        // its
        // subcomponents is being accessed. Input0 is a pointer to the beginning of the
        // structure,
        // and input1 is a byte offset to the subcomponent. As an operation, PTRSUB
        // produces a
        // pointer to the subcomponent and stores it in output.

        BitVecExpr i = ctx.mkBVAdd(pExpr, oExpr);
    ...
    public String toString() {
        return new StringBuilder("PTRSUB[").append(pointer.toString()).append("+").append(offset.toString()).append("]")
                .toString();
    }
