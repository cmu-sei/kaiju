...
public class ShrExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BitVecExpr instantiate(GhiHornContext ctx) {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Shr requires bitvector term: " + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Shr requires bitvector term: " + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVLSHR(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public ShrExpression(HornExpression lhs, HornExpression rhs) {
        this.lhs = lhs;
        this.rhs = rhs;
    }
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof ShrExpression))
            return false;
