...
public class SremExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BitVecExpr instantiate(GhiHornContext ctx) throws Z3Exception {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Srem requires bitvector term: " + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Srem requires bitvector term: " + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVSRem(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public SremExpression(HornExpression lhs, HornExpression rhs) {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof SremExpression))
            return false;
        SremExpression other = (SremExpression) obj;
        if (lhs == null) {
            if (other.lhs != null)
                return false;
