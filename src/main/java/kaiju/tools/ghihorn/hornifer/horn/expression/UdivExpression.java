...
public class UdivExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BitVecExpr instantiate(GhiHornContext ctx) throws Z3Exception {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Udiv requires bitvector term: " + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Udiv requires bitvector term: " + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVUDiv(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public UdivExpression(HornExpression lhs, HornExpression rhs) {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof UdivExpression))
            return false;
        UdivExpression other = (UdivExpression) obj;
        if (lhs == null) {
            if (other.lhs != null)
                return false;
