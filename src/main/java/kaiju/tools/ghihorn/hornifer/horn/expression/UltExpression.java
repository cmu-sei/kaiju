...
public class UltExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BoolExpr instantiate(GhiHornContext ctx) {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Ult requires bitvector term: " + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Ult requires bitvector term: " + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVULT(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public UltExpression(HornExpression lhs, HornExpression rhs) {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof UltExpression))
            return false;
        UltExpression other = (UltExpression) obj;
        if (lhs == null) {
            if (other.lhs != null)
                return false;
