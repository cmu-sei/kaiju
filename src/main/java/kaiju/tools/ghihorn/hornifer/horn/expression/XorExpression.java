...
public class XorExpression implements HornExpression {
    private final HornExpression lhs, rhs;

    @Override
    public BitVecExpr instantiate(GhiHornContext ctx) throws Z3Exception {

        Verify.verify(lhs.getType() == GhiHornType.BitVec, "Xor requires bitvector term: " + lhs);
        Verify.verify(rhs.getType() == GhiHornType.BitVec, "Xor requires bitvector term: " + rhs);

        BitVecExpr lhsExpr = (BitVecExpr) lhs.instantiate(ctx);
        BitVecExpr rhsExpr = (BitVecExpr) rhs.instantiate(ctx);

        return ctx.mkBVXOR(lhsExpr, rhsExpr);
    }
     ...
     * @param rhs
     */
    public XorExpression(HornExpression lhs, HornExpression rhs) {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof XorExpression))
            return false;
